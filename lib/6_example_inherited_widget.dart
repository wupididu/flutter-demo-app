...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    // Examples of InheritedWidgets

    // Можно достать какие-то данные из MediaQuery
    // Под капотом вызывается [dependOnInheritedWidgetOfExactType]
    final mq = MediaQuery.of(context);
    mq.size;

    // Еше один пример InheritedWidget, где мы подписываемся на изменения
    // темы в дереве виджетов.
    //Под капотом тоже вызывается [dependOnInheritedWidgetOfExactType]
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) => index == 0
            ...
            : PostItem(
                ...
                key: ValueKey(likedPosts[index - 1]),
